...
Every collection and global in the Payload admin panel is functionally sound, but the sidebar gives editors almost no context: 18 of 21 nav-registered items (14 collections, 5 globals, minus the 3 globals that already have descriptions) have no `admin.description`, `products` is missing its "view live" link, and the dashboard landing page omits four real content collections from its quick-links entirely. This project fixes navigation clarity and link correctness — it does not remove any collection or global, since the audit found nothing genuinely dead.

> **Revision note (post-approval):** this spec was originally drafted against a stale pre-review snapshot of `feat/env-validation-events-reskin`, where `Events.ts` had a broken preview config. By the time implementation started, PR #217's own code review had already fixed that — by deliberately *removing* the preview/livePreview config rather than building a detail route, with a comment explaining why (`no per-event detail route exists ... revisit if an /events/[slug] detail page ships`). Re-confirmed with the project owner: leave that decision as-is. The "Events gets a real detail page" scope item and the original section 3 below are **superseded** — struck through, not deleted, so the reasoning trail stays visible.
...
- **`products` has no working preview link.** Its `admin.livePreview` config sets only `interval`, with no `url` function and no `admin.preview` — so there is no "view live" button in the document editor at all, unlike `pages`, `posts`, and `how-to-guides`, which all wire `generatePreviewPath()`.
- ~~**`events`'s preview link is broken.**~~ **Superseded.** At spec-drafting time, `Events.ts` called `generatePreviewPath({ collection: 'events', ... })` against a `collectionPrefixMap` that didn't include `events`, resolving to a broken `undefined/<slug>` URL, with no `/events/[slug]` route to point to anyway. PR #217's review already fixed this by removing the preview/livePreview config outright (see revision note above) — `events` is now correctly preview-less, matching its actual route surface (`/events` listing only).
...
4. ~~**Events gets a real detail page.** Rather than just removing the broken preview config, a new `/events/[slug]` route is built so the existing preview/live-preview wiring in `Events.ts` resolves to something real.~~ **Superseded** — see revision note. Events keeps its current, deliberately preview-less config; no new route is built as part of this cleanup.
...
### 3. ~~Fix `events` preview link + build the detail page~~ Superseded — no longer part of this project

Already resolved upstream by PR #217's own review (preview config removed rather than pointed at a nonexistent route). No `/events/[slug]` route is built here. Note for future reference: `src/utilities/revalidate.ts` already registers an `events-revalidation` rule targeting `/events/${slug}` and `/events` — currently a no-op for the per-slug path since no such route exists, but nothing to fix; it'll start working the moment a detail route ships.
...
- Building `/events/[slug]` — PR #217 review already decided against it (preview config deliberately omitted); revisit only if that decision changes.

## Testing

- **Types & lint:** `pnpm generate:types` (no schema change, but confirms nothing broke) and `vp check`.
- **Int test** (`tests/int/`): extend or add a spec asserting `generatePreviewPath({ collection: 'products', slug, req })` returns a path containing `/products/` (regression guard against the same class of `undefined/<slug>` bug that Events previously had).
- **Manual verification via dev server:**
  ...
  3. Dashboard home: confirm the "More Content" row now includes Events, How-To Guides, Scent Quiz, and Scent Profiles links, and each resolves to the correct admin list view.
