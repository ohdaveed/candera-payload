...
export function useTask({
  ...
  const startStream = useCallback(() => {
    ...
    void manager.start()
  }, [taskId, client, onUpdate, onComplete, onError, stopStream])

  const fetchTask = useCallback(async () => {
    if (!taskId || !client) return

    // Defer state updates to avoid setting state synchronously in an effect
  ...
  }, [taskId, fetchTask, stopStream]) // oxlint-disable-line react-hooks/exhaustive-deps
