...
const priority = enumOf(['low', 'medium', 'high', 'critical'], 'Task priority level')
```
...
// Basic tool
const greet = tool('greet').describe('Greets a user').param('name', string('Name to greet')).build()
...
```typescript
import type { AgentTool, ToolCall, ToolResult, ParamType } from '@inferencesh/sdk'
...
// Built-in tools
const internals = internalTools().webSearch(true).codeExecution(true).build()
...
// Handle tool calls
const response = await agent.sendMessage('Calculate 15% tip on $85, then notify Slack', {
  onToolCall: async (call) => {
