...
2. Verify prod env vars are non-empty via `vercel env pull --environment=production` (plain `vercel env pull` pulls development vars, not production).
...
When setting Vercel env vars, always verify the value actually persisted with a follow-up `vercel env pull --environment=production` (plain `vercel env pull` pulls development vars) or a read-back check before claiming success — prior attempts reported success while variables remained empty. Confirm production `DATABASE_URI`/`POSTGRES_URL` are non-empty before declaring DB connectivity restored.

## Commit Hygiene

`src/app/(payload)/admin/importMap.js` is generated by `pnpm generate:importmap` and is tracked in git — regenerate and commit it whenever an admin-visible field or component changes, rather than leaving the bundle pointed at stale imports. Don't let it ride along uncommitted (or committed) in unrelated diffs — regenerate deliberately, stage only intended files, and check `git status` before committing. Never disable or bypass ggshield/security git hooks globally — scope `hooksPath` locally if needed.
