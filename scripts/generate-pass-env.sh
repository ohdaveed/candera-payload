#!/usr/bin/env zsh
# Generate pass:// references for all items/fields in a Proton Pass vault (zsh)
# Usage: ./scripts/generate-pass-env.sh --vault-name <VAULT_NAME> [--out <file>] [--dry-run]
...
usage() {
  cat <<EOF
  Usage: $0 --vault-name <VAULT_NAME> [--out <file>] [--dry-run]

  Generates lines of the form:
    ENV_VAR="pass://<SHARE_ID>/<ITEM_ID>/<field>"

    Options:
      --vault-name  Vault name to inspect (required; e.g. "Google")
                                                                                  ...
                                                                                  export PROTON_PASS_AGENT_REASON="Generate pass env mappings"
                                                                                    ...
                                                                                    sanitize() {
                                                                                      # Uppercase, replace non-alnum with _, collapse underscores
                                                                                        ...
                                                                                        echo "Listing items in vault '${VAULT_NAME}'..." >&2
                                                                                        ITEMS_JSON=$(pass-cli item list --vault-name "$VAULT_NAME" --output json) || {
                                                                                          echo "pass-cli failed to list items for vault '${VAULT_NAME}'" >&2
                                                                                            rm -f "$TMP_OUT"
                                                                                              exit 4
                                                                                              }

                                                                                              # Resolve the share ID for the vault so we can build pass:// URIs
                                                                                              SHARE_ID=$(pass-cli vault list --output json 2>/dev/null | jq -r --arg name "$VAULT_NAME" '.vaults[] | select(.name == $name) | .shareId // .share_id // .id // empty' | head -1)
                                                                                              if [[ -z "$SHARE_ID" ]]; then
                                                                                                echo "Warning: could not resolve share ID for vault '${VAULT_NAME}'; pass:// URIs will use vault name as placeholder" >&2
                                                                                                  SHARE_ID="$VAULT_NAME"
                                                                                                  fi
